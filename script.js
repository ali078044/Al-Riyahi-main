...
const API_URL = 'https://script.google.com/macros/s/AKfycbyEAsVbqIPuWdVoZzc0u7YCT_EuAT7t2dUbkhi9RDTX78Hc_yQ4giMunC-qcaVXQsIF/exec'; 
// ==========================================================
//                   دوال الأمان (جديد)
// ==========================================================
...
function handleTeacherPage() {
    ...
    // تحميل الاعتراضات المحولة للمدرس
    loadTeacherObjections();

    // (جديد) تحميل الواجبات المرسلة وسجل المشاهدات عند فتح الصفحة
...
function downloadExcelTemplate() {
    const headers = [["ID الطالب", "شهر 1 - ف1", "شهر 2 - ف1", "نصف السنة", "شهر 1 - ف2", "شهر 2 - ف2", "الامتحان النهائي"]];
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.aoa_to_sheet(headers);
    XLSX.utils.book_append_sheet(wb, ws, "الدرجات");
    XLSX.writeFile(wb, "نموذج_درجات_الرياحي.xlsx");
}

function cancelExcelUpload() {
    document.getElementById('excelPreviewSection').style.display = 'none';
    document.getElementById('studentsGradeContainer').innerHTML = '';
    document.getElementById('excelFileInput').value = '';
}

// دالة معالجة رفع ملف الإكسل وجلب أسماء الطلاب للمطابقة
async function handleExcelUpload(e) {
    ...
    // التحقق من اختيار الصف والمادة أولاً لضمان جلب الأسماء الصحيحة
    if (!studentClass || !subject) {
        Swal.fire('تنبيه', 'يرجى اختيار الصف والمادة أولاً من القوائم لكي نتمكن من مطابقة الأسماء.', 'warning');
        e.target.value = '';
        return;
    }

    Swal.fire({ title: 'جاري جلب بيانات الطلاب وتحليل الملف...', allowOutsideClick: false, didOpen: () => { Swal.showLoading(); } });

    // جلب قائمة الطلاب الحالية لهذا الصف لمطابقة الـ ID بالأسماء
    const result = await callApi('getStudentsForGrading', { studentClass, studentSection: 'الكل', subject });
    const studentsMap = {};
    if (result.success && result.students) {
        result.students.forEach(s => {
            studentsMap[s.studentId] = s.name;
        });
    }
    ...
    reader.onload = (ev) => {
        try {
            ...
            if (jsonData.length === 0) {
                Swal.fire('خطأ', 'الملف المرفوع فارغ.', 'error');
                return;
            }

            renderGradesFromExcel(jsonData, studentsMap);
            Swal.close();
        } catch (err) {
            console.error(err);
            Swal.fire('خطأ', 'حدثت مشكلة أثناء قراءة ملف الإكسل.', 'error');
        }
    };
    reader.readAsArrayBuffer(file);
}

// دالة رسم الجدول ومعاينة البيانات مع إظهار أسماء الطلاب
function renderGradesFromExcel(excelData, studentsMap) {
    ...
    const mainContainer = document.getElementById('studentsGradeContainer');
    ...
    excelData.forEach(row => {
        // محاولة إيجاد ID الطالب من الأعمدة المختلفة
        const sId = row['ID الطالب'] || row['ID'] || row['StudentID'] || Object.values(row)[0];
        // جلب الاسم من الخريطة التي تم جلبها من السيرفر
        const sName = studentsMap[sId] || '<span style="color:red; font-weight:bold;">ID غير موجود بالنظام</span>';
        
        if (sId) {
            html += `
                <tr data-student-id="${sId}">
                    <td><strong>${sId}</strong></td>
                    <td style="text-align:right; background:#f9f9f9;">${sName}</td>
                    <td><input type="number" class="grade-input" data-grade-type="Term1_Month1" value="${row['شهر 1 - ف1'] || ''}"></td>
                    <td><input type="number" class="grade-input" data-grade-type="Term1_Month2" value="${row['شهر 2 - ف1'] || ''}"></td>
                    <td><input type="number" class="grade-input" data-grade-type="MidYear_Exam" value="${row['نصف السنة'] || ''}"></td>
                    <td><input type="number" class="grade-input" data-grade-type="Term2_Month1" value="${row['شهر 1 - ف2'] || ''}"></td>
                    <td><input type="number" class="grade-input" data-grade-type="Term2_Month2" value="${row['شهر 2 - ف2'] || ''}"></td>
                    <td><input type="number" class="grade-input" data-grade-type="Final_Exam" value="${row['الامتحان النهائي'] || ''}"></td>
                </tr>`;
        }
    });
    ...
    // عرض المعاينة وتحديث الحاوية الرئيسية للإرسال
    container.innerHTML = html;
    previewSection.style.display = 'block';
    mainContainer.innerHTML = html;
    document.getElementById('submitGradesBtn').style.display = 'block';
    
    // تمرير الشاشة للمعاينة تلقائياً
    previewSection.scrollIntoView({ behavior: 'smooth' });
}

// دالة تحميل النموذج الفارغ للمدرسين
function downloadExcelTemplate() {
    const headers = [["ID الطالب", "شهر 1 - ف1", "شهر 2 - ف1", "نصف السنة", "شهر 1 - ف2", "شهر 2 - ف2", "الامتحان النهائي"]];
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.aoa_to_sheet(headers);
...
// دالة إلغاء عملية الرفع وتفريغ الحاويات
function cancelExcelUpload() {
    document.getElementById('excelPreviewSection').style.display = 'none';
    document.getElementById('studentsGradeContainer').innerHTML = '<p>الرجاء اختيار الصف والشعبة والمادة لعرض الطلاب.</p>';
    document.getElementById('excelFileInput').value = '';
    document.getElementById('submitGradesBtn').style.display = 'none';
    Swal.fire('تم الإلغاء', 'تم تفريغ بيانات الملف المرفوع.', 'info');
}